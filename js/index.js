...
function showDetail(obj){
	...
	if(usedDtail[1]=="誕生"){
		...
		console.log("入ってる"+usedDtail[5]+"aa");
...
app.controller("ctrl", function ($scope,$timeout) {
	// ユーザー情報取得
	$.ajax({
		url: "api/index.php?state=getUserInfo",
		cache: false,
		async:false,
...
// 写真の更新
    ...
    $scope.updatePicture = function() {
		...
		var date = new Date();
		$('#profileImage').attr('src', "img/" + $scope.userInfo.id + ".jpg?" + date.getTime());
	...
	$scope.changeIsPublic = function(){
		$scope.add.isPublic=!$scope.add.isPublic;
	};
	...
	$scope.addStory = function(){
	    ...
	    $.ajax({
			type: "POST",
			url: "api/index.php?state=addStory",
			cache: false,
			async:false,
			data: {
